...
class ParamGaussian(torch.distributions.Distribution):
    ...
    def log_prob(self, z):
        normal_dist = torch.distributions.Distribution.normal.Normal(
            loc=self.mu, scale=self.sigma, validate_args=False
        )
...
class Decoder(nn.Module):
    def __init__(self, output_size, latent_size):
        ...
        self.decoder_conv = nn.Sequential(
            ...
            # [64,13,13] -> [32,25,25]
            nn.ConvTranspose2d(
                in_channels=64, out_channels=32, kernel_size=5, stride=2, padding=2
            ),
            nn.BatchNorm2d(32),
            nn.LeakyReLU(),
            # [32,25,25] -> [3,50,50]
            nn.ConvTranspose2d(
                in_channels=32, out_channels=3, kernel_size=6, stride=2, padding=2
            ),
        )
...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size):
        ...
        self.register_buffer(
            "prior_params", torch.zeros(torch.Size([1, 2 * self.latent_size]))
        )
